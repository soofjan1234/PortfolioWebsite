...
Create Excalidraw diagrams from text content.
...
#### 3. 使用 Write 工具自动保存文件
- **保存位置**：`{current_directory}/Excalidraw/[filename].md`
- **自动创建目录**：如果 `Excalidraw` 目录不存在，会自动创建

#### 4. 检查并参考现有风格
- **统一风格**：
  - 检查配色方案（是否使用主题色 `#38bdf8`、`#4ade80` 等）
  - 检查字体样式（是否使用 `ExLingHei Light` 或其他字体）
  - 检查图形样式（圆角矩形、箭头样式等）
  - 检查布局习惯（对齐方式、间距等）

#### 5. 确保 Markdown 结构完全正确
**必须按以下格式生成**（不能有任何修改）：
