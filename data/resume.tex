...
\begin{itemize}[parsep=0.3ex]
    \item \textbf{编程语言与框架}：使用 Go（Gin、Gorm） 开发，系统掌握 Goroutine/Channel、GMP 调度；拥有 Vue 实战经验，具备多端适应能力。
    \item \textbf{数据存储与缓存}：掌握 MySQL 存储引擎（InnoDB）、B+ 树索引、锁与事务（ACID、隔离级别、MVCC）；掌握 Redis 底层数据结构、持久化，能应对缓存穿透、击穿、雪崩等问题。
    \item \textbf{中间件}：熟悉 NSQ 架构，能处理消息丢失与重复消费（幂等设计）；了解Kafka、RocketMQ等消息队列在不同场景下的使用选择。
