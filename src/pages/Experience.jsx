    ...
    const projectModules = [
        ...
        },
        {
            title: "智能文档中心", details: [
                ...
                "针对超长文档，设计了基于块的索引策略与搜索结果聚合算法，为用户提供了具备高上下文关联性的「搜索高亮快照」。",
                "支持 6 种文档格式解析，通过 chardet 解决跨编码乱码问题，实现全量 UTF-8 归一化。"
            ]
        },
        {
            title: "文件索引同步系统", details: [
                "从 fsnotify 事件监听模型切换至自研 BFS+Queue 架构，在海量文件场景下内存开销降低 90% 以上。",
                "基于接口抽象与 Functional Options 模式，实现同步逻辑与底层存储、IO 的深度解耦，支持跨项目的高内聚集成。",
                "实现基于 SQLite 状态机的增量同步引擎，包括断点续传及冗余过滤，显著提升同步可靠性与响应效率。"
            ]
        },
        {
            title: "AI相册", details: [
                "图片分类与人脸识别：通过 DBSCAN 聚类算法实现万级人脸库的无监督聚类，支持动态聚类中心更新，识别准确率达 97.3%。",
                "后端：在检索模块中，基于策略模式实现 FileRetriever 接口，支持独立部署与依赖网盘两种检索策略的运行时切换，满足不同部署场景。"
            ]
        },
        {
